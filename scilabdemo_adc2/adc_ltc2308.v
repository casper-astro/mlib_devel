...
begin
	...
	begin
		...
		write_pos <= data_bits_num_holder - 1; //4'd4;
	end
	else if (clk_enable)
	begin
...
always @(negedge reset_n)
begin
	config_cmd <= {4'h8, `UNI_MODE, `SLP_MODE};
	//if (~reset_n)
	//begin
	//case (measure_ch)
...
//localparam[2:0] cmd_bits_num_holder = `CMD_BITS_NUM;

assign config_init = (tick == `tCONFIG_START)?1'b1:1'b0;	
assign config_enable = (tick > `tCLK_START && tick <= `tCONFIG_END)?1'b1:1'b0;	// > because this is negative edge triggered
