...
begin
	...
	begin
		{continuous_mode, measure_fifo_ch, measure_fifo_start} <= slave_wriredata[4:0]; 
	end
...
always @ (posedge slave_clk or negedge adc_reset_n)	
begin
	if (~adc_reset_n)
	begin
		measure_start <= 1'b0;
	...
	begin
		...
		begin
			if (config_first)
			begin	
				// first conversion after (re)config is just to load the config
				config_first <= 1'b0;
				wait_measure_done <= 1'b0;
			end
			else
			begin	// read data and save into fifo
				if (continuous_mode) begin
						// in continuous mode we NEVER assert measure_fifo_done,
						// we only stop starting new conversions if the FIFO is full
						if (~fifo_wrfull) begin
							wait_measure_done <= 1'b0; // immediately start next conversion
						end
						// else: stay in wait_measure_done until FIFO has space
				end
				else
				begin
					// finite N-sample mode
					if (measure_count < (measure_fifo_num - 1)) // && ~fifo_wrfull)
					begin	
						measure_count <= measure_count + 12'd1;
						wait_measure_done <= 1'b0; // start the next conversion
					end
					else
					begin
						// we already collected measure_fifo_num samples
						measure_fifo_done <= 1'b1;
						// stay in this state until reset via adc_reset_n
					end
				end
			end
		end
...
assign fifo_wrreq = (~pre_measure_done & measure_done & ~config_first & ~fifo_wrfull)?1'b1:1'b0;
