...
function onehundred_gbe_callback()
% this function is reused from the ten_gbe_callback.
    clog('entering onehundred_gbe_callback', 'trace');
    
    blk = gcb;
    
    check_mask_type(blk, 'onehundred_gbe');
    ...
    if length(xps_xsg_blks) ~= 1,
      warndlg('onehundred_Gbe requires a single MSSGE (XSG core config) block to be instantiated at the top level');
      return;
    end
    ...
    clog('exiting onehundred_gbe_callback', 'trace');
