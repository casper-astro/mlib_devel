...
function b = xps_adc_3ghz(blk_obj)

if ~isa(blk_obj,'xps_block')
    error('XPS_ADC class requires a xps_block class object');
end

if ~strcmp(get(blk_obj,'type'),'xps_adc_3ghz')
...
b = class(s,'xps_adc_3ghz',blk_obj);

% ip name and version
b = set(b, 'ip_name', 'national_adc_interface');
switch s.hw_sys
    case 'iBOB'
...
ext_ports.adc_ddrb_p        = {1 'out' [s.adc_str,'ddrb_p']        ['{',adcport,'_p{[19]+1,:}}']                         'vector=false'  struct()        ucf_constraints_noterm };
