...
for n = 1:length(gateway_ins)
    gw = gateway_ins{n};
    if regexp(get_param(gw,'Name'),'(adc_user_data[iq]\d+)$')
        toks = regexp(get_param(gw,'Name'),'(adc_user_data[iq]\d+)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(adc_user_outofrange\d)$')
        toks = regexp(get_param(gw,'Name'),'(adc_user_outofrange\d)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(adc_user_sync\d)$')
        toks = regexp(get_param(gw,'Name'),'(adc_user_sync\d)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(adc_user_data_valid\d)$')
        toks = regexp(get_param(gw,'Name'),'(adc_user_data_valid\d)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    else
