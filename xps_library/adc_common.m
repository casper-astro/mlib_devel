...
function yoff = adc_common(blk, varargin)

  clog('adc_common: entering','trace');

  defaults = {'in', 1, 'out', 8, 'bits', 8, 'adc_interleave', 'off', ...
    'of_support', 'on', 'sync_support', 'on', 'dv_support', 'on', ...
    'of_per_input', 1};
  ...
  of_support = get_var('of_support', 'defaults', defaults, varargin{:});
  sync_support = get_var('sync_support', 'defaults', defaults, varargin{:});
  dv_support = get_var('dv_support', 'defaults', defaults, varargin{:});
  of_per_input = get_var('of_per_input', 'defaults', defaults, varargin{:});

  clog(['in: ',num2str(in),' out: ',num2str(out),' bits: ',num2str(bits),' adc_interleave: ',adc_interleave, ...
    ' of_support: ',of_support,' sync_support: ',sync_support,' dv_support: ',dv_support,' of_per_input: ',num2str(of_per_input)], 'adc_common_debug');

  %useful derivatives
  if strcmp(adc_interleave,'on'), il = 1; else, il = 0; end
  if strcmp(of_support,'on'), of = 1; else, of = 0; end
  ...
  for d = 0:in-1
    ...
    for ds = 0:out-1,
      ...
      reuse_block(blk, name, 'xbsIndex_r4/Gateway In', ...
        'arith_type', 'Signed', 'n_bits', num2str(bits), 'bin_pt', '0', ...  
      ...
      %output
      reuse_block(blk, ['data',num2str(d),'_',num2str(ds)], 'built-in/outport', 'Port', num2str(d*(out+of*of_per_input)+ds+1), ...
      ...
      if of == 1,
        ...
        if ds == out-1,
          ...
          for of_port = 0:of_per_input-1,
            gw = clear_name([blk, '_user_outofrange', num2str(d*of_per_input+of_port)]);
            ...
            clog(['out of range output port ',num2str(d*of_per_input+of_port),' at ',num2str(d*(out+of_per_input)+out+of_port+1)],'adc_common_detailed_trace');
            reuse_block(blk, ['or',num2str(d*of_per_input+of_port)], 'built-in/outport', 'Port', num2str(d*(out+of_per_input)+out+of_port+1), ...
              'Position', [xtick*13-15 ytick*(yoff+2+of_port)-7.5 xtick*13+15 ytick*(yoff+2+of_port)+7.5]);
            add_line(blk, [gw,'/1'], ['or',num2str(d*of_per_input+of_port),'/1']);
          end
        end 
      end %if of
  ...
  if sync == 1,
    ...
    for ds = 0:out/(2^il)-1,
      ...
      %output
      clog(['doing sync ', num2str(ds),' at ',num2str((in*out)+(of*in*of_per_input)+(ds+1))],'adc_common_detailed_trace');
      reuse_block(blk, ['sync',num2str(ds)], 'built-in/outport', 'Port', num2str((in*out)+(of*in*of_per_input)+(ds+1)), ...
  ...
  if dv == 1, 
    ...
    reuse_block(blk, 'sim_data_valid', 'built-in/inport', 'Port', num2str(in+2), 'Position', [xtick*1-15 ytick-7.5 xtick*1+15 ytick+7.5]);
    name = clear_name([blk, '_user_data_valid']);
    reuse_block(blk, name, 'xbsIndex_r4/Gateway In', ...
      'arith_type', 'Boolean', ...
      'Position', [xtick*8-20 ytick-10 xtick*8+20 ytick+10 ]);
    add_line(blk,'sim_data_valid/1', [name,'/1']);
    reuse_block(blk, 'data_valid', 'built-in/outport', 'Port', num2str(out*in+out/(2^il)*sync+(of*in*of_per_input)+1),...
