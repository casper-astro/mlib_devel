...
if run_drc
	...
	for i=1:length(xps_objs)
	    [result, msg] = drc(xps_objs{i},xps_objs);
	    if result
...
if run_ip
    ...
    cd(simulink_path);
    for i = 1:length(xps_pcore_blks)
        path_param = get_param(xps_pcore_blks(i), 'pcore_path');
    ...
    fprintf(mpd_fid,'###################\n');
    fprintf(mpd_fid,'PORT clk = "", DIR = I\n');
    fprintf(mpd_fid,'\n');
    fprintf(mpd_fid,'###################\n');
...
if run_elab
	...
	for i=1:length(xps_objs)
	    try
	        xps_objs{i} = elaborate(xps_objs{i});
	    catch
	        display(xps_objs{i})
	        disp(lasterr)
...
if run_software
	...
	switch sw_os
		case 'tinySH'
		    ...
		    for i=1:length(custom_xps_objs)
		        fprintf(nfo_fid, [get(custom_xps_objs{i},'type'), ',']);
		    end
			...
			for i = 1:length(commands)
				...
				commanddef_str   = ['static tinysh_cmd_t cmd_',num2str(i),' = {0,"',command{2},'","',command{3},'","',command{4},'",',command{1},',0,0,0};\n'];
				commandadd_str   = ['\ttinysh_add_command(&cmd_',num2str(i),');\n'];
			...
			for i=1:length(headers)
				headers_str = [headers_str,'Header: ',headers{i},10];
			end
			...
			sources_str = '';
			for i=1:length(sources)
				sources_str = [sources_str,'Source: ',sources{i},10];
			end
