  ...
  try
    ...
    if strcmp(our_port, their_port),   %same port
      if ~strcmp(our_name, their_name) % and name not the same
        msg = ['40Ge port ', our_name,' and 40Ge port ', their_name,' are located on the same port.'];
        result = 1;
      end
    end
    ...
    % the below will be implemented at a later stage
    %if strcmp(our_hw, 'SKARAB'),                % skarab
    ...
    %      msg = ['40Ge ports ''', our_name,''' and ''', their_name,''' are both located in mezzanine slot ',our_slot,', but have different mezzanine flavours.'];
    %      result = 1;
    %    end
    %  end
    end
