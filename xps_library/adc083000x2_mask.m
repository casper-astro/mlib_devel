...
for n = 1:length(gateway_ins)
    ...
    elseif regexp(get_param(gw,'Name'),'(adc\d+_user_outofrange\d)$')
        toks = regexp(get_param(gw,'Name'),'(adc\d+_user_outofrange\d)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(adc\d+_user_sync\d)$')
        toks = regexp(get_param(gw,'Name'),'(adc\d+_user_sync\d)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
