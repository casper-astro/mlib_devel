...
switch s.hw_sys
  ...
  case {'ROACH2','MKDIG'}, 
    ...
    %values below taken from ug366 transceiver user guide (should match with tengbe_loadfcn)
    ...
    if isempty(index), 
      error(['xps_tengbe:''',str2num(s.postemph_r2),''' not found in ''',postemph_lookup,'''']);
      return; 
    end
    ...
    if index == [], 
      error(['xps_tengbe:''',str2num(s.preemph_r2),''' not found in ''',preemph_lookup,'''']);
      return; 
    end
    ...
    if index == [], 
      error(['xps_tengbe:''',str2num(s.swing_r2),''' not found in ''',swing_lookup,'''']);
      return; 
    end
