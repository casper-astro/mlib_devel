...
class SimulinkFrontend(ToolflowFrontend):
    ...
    def _gen_hdl_simulink(self, hdl_sysgen_filename):
        ...
        :param hdl_sysgen_filename: This is the path and hdl file that
            contains the original sysgen code. This file is overwritten with new latency info before being imported to the
            compile directory
        :type filename_bin: str
        """
        ...
        if dpram_exists == True and ver_exists == True:
            ...
            with open(hdl_sysgen_filename, 'w') as fh2:
                fh2.writelines(lines)
            fh2.close()
            self.logger.info('File written. Vivado version is 2018.2: %s. Dual Port RAM exists: %s'
                             % (ver_exists, dpram_exists))
        else:
            self.logger.info('File not written. Vivado version is 2018.2: %s. Dual Port RAM exists: %s'
                             % (ver_exists, dpram_exists))
        #import IPython
        #IPython.embed()
