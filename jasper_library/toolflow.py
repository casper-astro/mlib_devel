	...
	def mkfpg(self, filename_bin, filename_fpg):
		...
		with open(extended_info, 'w') as fh4:
			...
			with open(basefile_design, 'r') as fh2:
				for line in fh2:
					fh4.write('?meta\t' + line.rstrip('\n') + '\n')

			with open(basefile_git, 'r') as fh3:
				for line in fh3:
					fh4.write(line.rstrip('\n') + '\n')
			'''
			with open(basefile_core, 'r') as fh1:
				for row in fh1:
					col1, col2, col3, col4 = row.split()
		...
		if self.name == 'quartus':
			dst_name = os.path.basename(filename_fpg) 
			#mkfpg_cmd4 = 'cp %s/extended_info.kcpfpg %s/%s' % (self.compile_dir, self.output_dir, filename_fpg)
			mkfpg_cmd4 = f'cp "{self.compile_dir}/extended_info.kcpfpg" "{self.output_dir}/{dst_name}"'
		else:
			mkfpg_cmd4 = 'cp %s/extended_info.kcpfpg %s/%s' % (self.compile_dir, self.output_dir, filename_fpg)
...
class QuartusBackend(ToolflowBackend):
	...
	def initialize(self):
		...
		if os.path.isdir(glue_dir):
			...
			self.logger.warning(f"No glues dir found at {glue_dir}")
	...
	def add_compile_cmds_pr(self, cores=8, plat=None, synth_strat=None, impl_strat=None):
		...
		#sof_orig = os.path.join(self.compile_dir, self.project_name, "outputs", self.project_name + ".sof") 
		...
		#tcl(f'file copy -force [file join $out myproj.sof] [file join $out top.sof]', stage = 'post_bitgen')
...
"""
	...
	def add_compile_cmds_pr(self, cores=8, plat=None, synth_strat=None, impl_strat=None):
		...
		ip_regexp = os.path.join(self.compile_dir, r"myproj/myproj.srcs/user_top-toolflow/ip/[^/]+/[^/]+\.xci")
