...
class ads5296x4(YellowBlock):
    def initialize(self):
        ...
        self.num_chips = self.num_units / self.n_inputs_per_chip
    ...
    def modify_top(self,top):
        ...
        inst.add_port('locked', 'adc16_locked', width=self.num_chips)
        ...
        for k in range(self.num_units):
            # Embedded wb-RAM
            bram_log_width = int(ceil(math.log(self.adc_data_width*4,2)))
            ...
            wbram.add_port('user_din',self.reorder_ports([din+'1',din+'2',din+'3',din+'4'], padding="%s'b0" % padding), parent_sig=False)
            wbram.add_port('user_we','adc16_snap_we')
            wbram.add_port('user_dout','')

    def reorder_ports(self,port_list, wb_bitwidth=32, padding=None):
