...
class au280(YellowBlock):
    def initialize(self):
        self.add_source('utils/cdc_synchroniser.vhd')
        self.add_source('au280')
        self.provides = [
            "sys_clk",
            "sys_clk90",
            "sys_clk180",
    ...
    def modify_top(self,top):
        inst = top.get_instance('au280_bd', 'au280_bd_inst')
        ...
        inst.add_port('sysclk0_clk_n',        'sysclk_clk_n',  dir='in', parent_port=True)
        inst.add_port('sysclk0_clk_p',        'sysclk_clk_p',  dir='in', parent_port=True)
        ...
        inst.add_port('satellite_gpio_0',     'satellite_gpio_0',     dir='in',  parent_port=True, width=4)
    ...
    def gen_constraints(self):
        ...
        #cons.append(ClockGroupConstraint('-of_objects [get_pins au280_inst/processing_system7_0/inst/PS7_i/FCLKCLK[0]]', '-of_objects [get_pins au280_infr_inst/dsp_clk_mmcm_inst/CLKOUT0]', 'asynchronous'))
    ...
    def gen_tcl_cmds(self):
        ...
        # import the xdc files associated with the au280 block diagram, pin outs and bit stream generation
        tcl_cmds['pre_synth'] += ['import_files -force -fileset constrs_1 %s/au280_infr/au280_bd.xdc'%os.getenv('HDL_ROOT')]
        tcl_cmds['pre_synth'] += ['import_files -force -fileset constrs_1 %s/au280_infr/au280_bitstream.xdc'%os.getenv('HDL_ROOT')]

        # run the block diagram generation script
        tcl_cmds['pre_synth'] += ['source {}'.format(self.hdl_root + '/au280_infr/au280_bd.tcl')]
        ...
        tcl_cmds['pre_synth'] += ['open_bd_design [get_files [get_property directory [current_project]]/myproj.srcs/sources_1/bd/au280_bd/au280_bd.bd]']
        ...
        tcl_cmds['pre_synth'] += ['close_bd_design [get_bd_designs au280_bd]']

        # generate the output files associated with the block diagram
        tcl_cmds['pre_synth'] += ['generate_target all [get_files [get_property directory [current_project]]/myproj.srcs/sources_1/bd/au280_bd/au280_bd.bd]']
        #tcl_cmds['pre_synth'] += ['make_wrapper -files [get_files [get_property directory [current_project]]/myproj.srcs/sources_1/bd/au280_bd/au280_bd.bd] -top']
        ...
        tcl_cmds['pre_impl'] += ['set_clock_groups -asynchronous -group [get_clocks axi_bram_ctrl_lmb_BRAM_PORTA_CLK] -group [get_clocks clk_out1_au280_bd_clk_wiz_0_0]']
