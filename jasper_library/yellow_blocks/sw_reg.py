    ...
    def modify_top(self,top):
        if self.blk['io_dir'] == 'To Processor':
            if self.platform.mmbus_architecture == 'AXI4-Lite':
                ...
                inst.add_port('IP_CLK',       signal='axi4lite_clk', parent_sig=False)
                inst.add_port('IP_RESET',     signal='axi4lite_rst', parent_sig=False)                
                inst.add_port('IP_BUS_VALID', signal='%s_in'%self.fullname, parent_sig=False)
                inst.add_port('IP_BUS',       signal='%s_in_we'%self.fullname, width=32, parent_sig=False)
                inst.add_port('OP_BUS',       signal='%s_user_data_in'%self.fullname, width=32, parent_sig=True)
            else:
            ...
            if self.platform.mmbus_architecture == 'AXI4-Lite':
                ...
                inst.add_port('IP_BUS_VALID', signal='0\'b01', parent_sig=False)
                inst.add_port('IP_BUS',       signal='%s_user_data_out'%self.fullname, width=32, parent_sig=True)
                #inst.add_port('OP_BUS',       signal='%s_OP_BUS'%self.fullname, width=32, parent_sig=True)
                inst.add_port('OP_BUS',       signal='%s_out'%self.fullname, width=32, parent_sig=True)
            else:
