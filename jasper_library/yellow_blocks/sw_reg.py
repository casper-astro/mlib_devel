    ...
    def modify_top(self,top):
        if self.blk['io_dir'] == 'To Processor':
            if self.platform.mmbus_architecture == 'AXI4-Lite':
                ...
                inst.add_port('IP_BUS_VALID', signal='%s_IP_BUS_VALID'%self.fullname, parent_sig=True)
                inst.add_port('IP_BUS',       signal='%s_IP_BUS'%self.fullname, width=32, parent_sig=True)
                inst.add_port('OP_BUS',       signal='%s_user_data_in'%self.fullname, width=32, parent_sig=False)
            ...
            if self.platform.mmbus_architecture == 'AXI4-Lite':
                ...
                inst.add_port('IP_BUS',       signal='%s_user_data_in'%self.fullname, width=32, parent_sig=False)
