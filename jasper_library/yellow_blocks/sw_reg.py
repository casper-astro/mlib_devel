    ...
    def modify_top(self,top):
        if self.blk['io_dir'] == 'To Processor':
            if self.platform.mmbus_architecture == 'AXI4-Lite':
                ...
                inst.add_port('OP_BUS',       signal='%s_in'%self.unique_name, width=32, parent_sig=True)
            else:
            ...
            if self.platform.mmbus_architecture == 'AXI4-Lite':
                ...
                inst.add_port('IP_BUS_VALID', signal='%s_out_we'%self.unique_name, parent_sig=False)
                inst.add_port('IP_BUS',       signal='%s_out'%self.unique_name, width=32, parent_sig=True)
