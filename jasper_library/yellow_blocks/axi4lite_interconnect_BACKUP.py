		...
		for key, val in self.memory_map.items():
			for reg in val["memory_map"]:
				if reg.axi4lite_mode != 'raw' and reg.ram:
					inst.add_port('axi4lite_%s_%s_add'      % (key, reg.name), '%s_%s_add'      % (key, reg.name), dir='in',   width=int(math.log2((reg.nbytes * 8)/reg.data_width)), parent_sig=True)
	...
	def gen_custom_hdl(self):
		...
		for key, val in list(self.memory_map.items()):
			for reg in val["memory_map"]:
				if reg.axi4lite_mode != 'raw':
					if reg.ram:
						axi4lite_wrapper.add_port('axi4lite_%s_%s_add'      %(key, reg.name), '%s_%s_addr'      %(key, reg.name), dir='in',  width=int(math.log2((reg.nbytes * 8)/reg.data_width)), parent_sig=True)
		...
		for key, val in list(self.memory_map.items()):
			if val['memory_map'][-1].axi4lite_mode != 'raw':
				if val['memory_map'][-1].ram:
					...
					self.ic_ports.append(Port('%s_%s_add'  %(key, regname), 'axi4lite_%s_%s_add'                                %(key, regname)))
