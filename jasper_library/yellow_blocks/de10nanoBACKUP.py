...
class de10nano(YellowBlock):
    def initialize(self):
        ...
        self.provides.append('axil_rst_n')
    ...
    def modify_top(self, top):
        ...
        top.add_signal("axil_rst", width=0)
        #top.add_signal('h2f_rst_n', width=1)
        top.add_signal('axil_clk', width=0)
        top.add_signal('axil_rst_n', width=0)
        top.add_signal('h2f_rst_n_int', width=0)
         # Optional: expose the HPS LW clock coming from the wrapper
        #top.add_signal('h2f_lw_axi_clock', width=1)
        ...
        # Clock/reset strategy for bring-up:
        # - sys_clk is still the external 50 MHz for timing reference
        # - axil_clk comes from HPS lightweight bridge clock
        # - sys_rst derived from the HPS reset (active-low)
        top.assign_signal('sys_clk', 'fpga_clk1_50')
        top.assign_signal('axil_clk', 'fpga_clk1_50')

        #top.assign_signal('axil_rst_n', 'h2f_rst_n_int')
        top.assign_signal('sys_rst', '~axil_rst_n')
        #top.assign_signal('M_AXI_awvalid', 'M_AXI_wvalid')
        #top.assign_signal('M_AXI_awaddr',  'M_AXI_awaddr')
        # You can add user_clk later; for now keep everything in axil_clk domain-        # Assign fpga_clk1_50 to internal clocks
        top.assign_signal("axil_rst_n", "h2f_rst_n_int")
        ...
        #top.add_raw_string("assign awprot_000 = 3'b000;")
        #top.add_raw_string("assign arprot_000 = 3'b000;")    

        top.assign_signal('arprot_000', "3'b000")
        #Removed 
        #   .f2h_axi_clk(axil_clk), 

        #top.add_raw_string("assign axil_rst = ~axil_rst_n;")
        #top.add_raw_string("assign M_AXI_awvalid = M_AXI_wvalid;")
        #top.add_raw_string("assign axil_rst = ~axil_rst_n;")

        top.add_raw_string(r"""
        de10nano #(
            .F2S_Width(1),
            .S2F_Width(0)
        ) de10nano_inst (
          // HPS lightweight bridge clock & reset
          .f2h_axi_clk (axil_clk),  // e.g. same 50 MHz
          .h2f_lw_BID   (12'b0),  // tie off
          .h2f_lw_RID   (12'b0),  // tie off
          .h2f_lw_RLAST (1'b1),   // single-beat reads
          .h2f_lw_axi_clk (axil_clk),
          .h2f_rst_n      (h2f_rst_n_int),

          // H2F-LW AXI4-Lite master from HPS into your fabric
          .h2f_lw_AWADDR  (M_AXI_awaddr[20:0]),
          .h2f_lw_AWPROT  (awprot_000),
          .h2f_lw_AWVALID (M_AXI_awvalid),
          .h2f_lw_AWREADY (M_AXI_awready),
          ...
          .h2f_lw_ARADDR  (M_AXI_araddr[20:0]),
        ...
        top.add_raw_string("""
           assign M_AXI_awaddr[31:21] = 11'b0;
           assign M_AXI_araddr[31:21] = 11'b0;
        """)
    ...
    def gen_constraints(self):
        ...
        # Constrain the external 50MHz pin to sys_clk only.
        # axil_clk is driven internally by the HPS LW bridge clock.
        cons.append(ClockConstraint('fpga_clk1_50', 'sys_clk', period=self.T_pl_clk_ns, port_en=True, virtual_en=False))
    ...
    def gen_tcl_cmds(self):    
        ...
        svLoc = '/home/bgodfrey/CASPER/mlib_devel/jasper_library/hdl_sources/de10nano/de10nano_hps_0_fpga_interfaces.sv'
        tcl_cmds['init'] += [
                            f"set_global_assignment -name SYSTEMVERILOG_FILE {svLoc}",
                            f"set_global_assignment -name VERILOG_FILE {svLoc}"]
        '''
        tcl_cmds['init'] = []
        tcl_cmds['create_bd'] = []
        tcl_cmds['pre_synth'] = []
