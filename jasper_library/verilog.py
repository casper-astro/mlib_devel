...
class VerilogModule(object):
    ...
    def gen_wbs_arb(self, ):
        ...
        for signal in output_signals:
            ...
            for i, arbiter in enumerate(self.arbiters):
                wbs_parent_arbiter += '%s_arb%s, ' %(signal, i)
        ...
        fh_new = open('%s/jasper_library/hdl_sources/wbs_arbiter/wbs_arbiter.v'%os.getenv('MLIB_DEVEL_PATH'), 'w')
        fh_new.write(wbs_parent_arbiter)
        fh_new.close()

        #self.add_sourcefile('%s/myproj/myproj.srcs/sources_1/imports/wbs_arbiter/wbs_arbiter.v'%self.compile_dir)
