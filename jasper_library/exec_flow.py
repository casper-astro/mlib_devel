    ...
    if opts.backend or opts.software:
        ...
        if opts.software:
            ...
            if platform.boot_image == 'toolflow':
                ...
                With the vitis option the `jasper.dtsi` that was generated by the toolflow will
                be compiled with `dtc` producing a compatible device tree overlay.
                """
                if opts.vitis:
                  dts_file = backend.dts_loc
                  backend.output_dtbo = tf.frontend_target_base[:-4] + '_%d-%02d-%02d_%02d%02d.dtbo' % (
                      tf.start_time.tm_year, tf.start_time.tm_mon, tf.start_time.tm_mday,
                      tf.start_time.tm_hour, tf.start_time.tm_min)

                  os.system('dtc -I dts {:s} -O dtb -b 0 -@ -o {:s}'.format(dts_file,
                      os.path.join(backend.output_dir, backend.output_dtbo)))
                  #backend.mkdto()
                  print('Created %s/%s' % (backend.output_dir, backend.output_dtbo))
                #########################################################################################
