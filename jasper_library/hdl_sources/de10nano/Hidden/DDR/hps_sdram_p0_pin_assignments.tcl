...
if {[llength $argument_list] == 1 } {
 ...
 # If this script is called from outside quartus_sh, it will re-launch itself in quartus_sh (shell is enough for QSF edits)
if { ![info exists quartus(nameofexecutable)] || ($quartus(nameofexecutable) != "quartus_sh") } {
	post_message -type info "Restarting in quartus_sh..."
...
# Run only if TimeQuest commands exist AND a netlist already exists.
if { [llength [info commands timing_netlist_exist]] } {
     if { [timing_netlist_exist] } {
         # ok ? nothing to do; netlist is present
     } else {
         # Skip creating one here; this script only needs to push QSF assignments.
         post_message -type info "No post-map timing netlist; skipping TimeQuest-only steps."
     }
 }
