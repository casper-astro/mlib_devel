...
for i =1:length(gateway_outs)
    ...
    if regexp(get_param(gw,'Name'),'(dac_data_i0)$')
        toks = regexp(get_param(gw,'Name'),'(dac_data_i0)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(dac_data_i1)$')
        toks = regexp(get_param(gw,'Name'),'(dac_data_i1)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(dac_data_q0)$')
        toks = regexp(get_param(gw,'Name'),'(dac_data_q0)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(dac_data_q1)$')
        toks = regexp(get_param(gw,'Name'),'(dac_data_q1)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(dac_sync_i)$')
        toks = regexp(get_param(gw,'Name'),'(dac_sync_i)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    elseif regexp(get_param(gw,'Name'),'(dac_sync_q)$')
        toks = regexp(get_param(gw,'Name'),'(dac_sync_q)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
...
for i =1:length(gateway_ins)
    ...
    if regexp(get_param(gw,'Name'),'(phase)$')
        toks = regexp(get_param(gw,'Name'),'(phase)$','tokens');
        set_param(gw,'Name',clear_name([gcb,'_',toks{1}{1}]));
    else
        error(['Unkown gateway name: ',gw]);
    end
