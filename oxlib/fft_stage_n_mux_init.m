...
check_mask_type(blk, 'fft_stage_n_mux');
...
delay_bit_limit = get_var('delay_bit_limit', 'defaults', defaults, varargin{:});
...
for i=1:length(delays),
    ...
    if strcmp(delays_bram,'on'),
        ...
        set_param(full_path, 'DelayLen', '2^(FFTSize-FFTStage+mux_factor)');
...
reuse_block( blk, 'butterfly_direct_mux', 'ox_lib/butterfly_direct_mux', ...
    'biplex', 'on', 'Coeffs', tostring(Coeffs), 'StepPeriod', tostring(StepPeriod), ...
    'Position', [435 32 530 118]);
    propagate_vars([blk,'/butterfly_direct_mux'], 'defaults', defaults, varargin{:});
