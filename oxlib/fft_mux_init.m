...
if( strcmp(specify_mult, 'on') && length(mult_spec) ~= FFTSize ),
    error('fft_mux_init.m: Multiplier use specification for stages does not match FFT size');
    return
end

biplexes = find_system(blk, 'lookUnderMasks', 'all', 'FollowLinks','on','masktype', 'fft_biplex_mux');
...
elseif n_inputs == FFTSize,
    pos = [400 20 520 175];
    reuse_block(blk, 'fft_direct', 'ox_lib/fft_direct', ...
    ...
    pos = [400 20 520 175];
    reuse_block(blk, 'fft_direct_mux', 'ox_lib/fft_direct_mux', ...
