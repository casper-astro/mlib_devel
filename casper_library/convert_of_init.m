...
%pipeline latencies
% if we have a lot of latency, allow inverts to have latency
% if we have a fair amount or more, allow final add to have latency

if latency > 2, invert_latency = 1;
else invert_latency = 0;
end

if latency > 1, and_latency = 1;
else and_latency = 0;
end
...
if wb_lost == 0,
	...
	reuse_block(blk, 'all_0s', 'xbsIndex_r4/Logical', ...
		...
		'latency', num2str(latency-(invert_latency+and_latency)), ...
	...
	for i = 1:(wb_lost+1),
		...
		reuse_block(blk, ['invert',num2str(i)], 'xbsIndex_r4/Inverter', ...
                  'latency', num2str(invert_latency), ...
