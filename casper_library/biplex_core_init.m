...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if strcmp(async, 'on'),
    reuse_block(blk, 'dvi', 'built-in/inport', 'Port', '5', 'Position', [15 158 45 172]);
  end

  n_bits = n_inputs*4;
  reuse_block(blk, 'Constant', 'xbsindex_r4/Constant', ...
      'arith_type', 'Unsigned', 'const', '0', 'n_bits', num2str(n_bits), 'bin_pt', '0', ...
  ...
  for stage = 1:FFTSize,
      ...
      % if coefficients occupy larger space than specified for this stage then store in BRAM
      if ((2^(stage - 1) * coeff_bit_width * 2) >= 2^coeffs_bit_limit), coeffs_bram = 'on';
  ...
  if strcmp(async, 'on'),
    add_line(blk, 'dvi/1', 'fft_stage_1/6');
    reuse_block(blk, 'dvo', 'built-in/outport', ...
        'Position', [x 158 x+30 172], ...
        'Port', '5');
    add_line(blk, [last_stage,'/5'], 'dvo/1');
  end

  clean_blocks(blk);

  fmtstr = sprintf('%d stages\nreduce %s', FFTSize, opt_target);
