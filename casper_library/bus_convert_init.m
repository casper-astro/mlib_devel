...
function bus_convert_init(blk, varargin)
 ...
 %default state, do nothing 
  ...
  for index = 1:compo,
    ...
    if(pt_out > pt_in), 
      if strcmp(of, 'on'),
        clog('Increasing binary points while detecting overflow not possible currently',{'error', 'bus_convert_init_debug'});
        error('bus_convert: Increasing binary points while detecting overflow not possible currently');
      else,
