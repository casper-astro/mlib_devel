...
function hilbert_init(blk, varargin)
  ...
  reuse_block(blk, 'bus_expand_a', 'casper_library_flow_control/bus_expand', ...
          'mode', 'divisions of equal size', ...
          'outputNum', '2', 'OutputWidth', num2str(BitWidth*n_inputs), ...
          'outputBinaryPt', '0', 'outputArithmeticType', '0', ...
          'Position', [140 29 190 76]);
  add_line(blk,'munge_a/1','bus_expand_a/1');
  
  reuse_block(blk, 'bus_expand_b', 'casper_library_flow_control/bus_expand', ...
          'mode', 'divisions of equal size', ...
          'outputNum', '2', 'OutputWidth', num2str(BitWidth*n_inputs), ...
          'outputBinaryPt', '0', 'outputArithmeticType', '0', ...
          'Position', [140 179 190 226]);
  ...
  reuse_block(blk, 'add_even_real', 'casper_library_bus/bus_addsub', ...
          'opmode', '0', ... %addition 
          'Position', [255 29 300 76]);

  reuse_block(blk, 'sub_odd_imag', 'casper_library_bus/bus_addsub', ...
          'opmode', '1', ... %subtraction
          'Position', [255 104 300 151]);

  reuse_block(blk, 'sub_even_imag', 'casper_library_bus/bus_addsub', ...
          'opmode', '1', ... %subtraction
          'Position', [255 179 300 226]);

  reuse_block(blk, 'add_odd_real', 'casper_library_bus/bus_addsub', ...
          'opmode', '0', ... %addition 
          'Position', [255 254 300 301]);

  for name = {'add_even_real', 'sub_odd_imag', 'sub_even_imag', 'add_odd_real'};
    set_param([blk,'/',name{1}], ...
          'n_bits_a', mat2str(repmat(BitWidth, 1, n_inputs)), 'bin_pt_a', num2str(BitWidth-1), 'type_a', '1', ...
          'n_bits_b', mat2str(repmat(BitWidth, 1, n_inputs)), 'bin_pt_b', num2str(BitWidth-1), 'type_b', '1', ...
          'n_bits_out', mat2str(repmat(BitWidth+1, 1, n_inputs)), 'bin_pt_out', num2str(BitWidth-1), 'type_out', '1', ...
  ...
  reuse_block(blk, 'bus_scale', 'casper_library_bus/bus_scale', ...
          'n_bits_in', mat2str(repmat(BitWidth+1, 1, n_inputs*4)), 'bin_pt_in', 'BitWidth-1', 'type_in', '1', ...
  ...
  reuse_block(blk, 'bus_convert', 'casper_library_bus/bus_convert', ...
          'n_bits_in', mat2str(repmat(BitWidth+1, 1, n_inputs*4)), 'bin_pt_in', num2str(BitWidth), ...
          'n_bits_out', num2str(BitWidth), 'bin_pt_out', num2str(BitWidth-1), ...
