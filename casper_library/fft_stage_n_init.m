...
defaults = {'FFTSize', 3, 'FFTStage', 1, 'input_bit_width', 18, 'coeff_bit_width', 18, ...
    'coeffs_bram', 'off', 'delays_bram', 'off', 'quantization', 'Round  (unbiased: +/- Inf)', ...
    'overflow', 'Wrap', 'add_latency', 1, 'mult_latency', 2, 'bram_latency', 2, ...
    'conv_latency', 1, 'arch', 'Virtex5', 'opt_target', 'logic', ...
    'use_hdl', 'off', 'use_embedded', 'off', 'dsp48_adders', 'off'};
...
reuse_block( blk, 'butterfly_direct', 'casper_library/FFTs/butterfly_direct', ...
    ...
    'coeffs_bram', tostring(coeffs_bram), 'conv_latency', num2str(conv_latency), ...
    'quantization', tostring(quantization), 'overflow', tostring(overflow), ...
    'arch', tostring(arch), 'opt_target', tostring(opt_target), 'use_hdl', tostring(use_hdl), ...
    'use_embedded', tostring(use_embedded), 'dsp48_adders', tostring(dsp48_adders));
