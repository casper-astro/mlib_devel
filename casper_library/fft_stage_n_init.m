...
delays_mem_type   = get_var('delays_mem_type', 'defaults', defaults, varargin{:});
...
if strcmp(async, 'on'), 
  ...
  reuse_block(blk, 'en_replicate0', 'casper_library_bus/bus_replicate', ...
    'replication', '2', 'csp_latency', num2str(latency), 'misc', 'off', 'Position', [95 455 125 475]);
...
if strcmp(async, 'on'),
  reuse_block(blk, 'en_replicate1', 'casper_library_bus/bus_replicate', ...
    'replication', num2str(outputNum), 'csp_latency', num2str(mux_latency), 'misc', 'off', 'Position', [395 466 425 484]);
...
if strcmp(delays_bram, 'on'),
  ...
  for n=1:2,
    ...
    if USE_DUMB_BRAMS == 0
      ...
      reuse_block(blk, delay, 'casper_library_bus/bus_single_port_ram', ...
        ...
        'max_fanout', num2str(max_fanout), 'mem_type', delays_mem_type, 'bram_optimization', 'Speed', ...
        'async', 'off', 'misc', 'off', ...
  ...
  reuse_block(blk, 'delay0', 'casper_library_bus/bus_delay', ...
    ...
    'csp_latency', '2^(FFTSize-FFTStage)', 'enable', async, 'reg_retiming', 'on', ...
  ...
  if strcmp(async, 'on'),   
    reuse_block(blk, 'den0', 'casper_library_bus/bus_replicate', ...
      'replication', num2str(n_inputs), 'csp_latency', num2str(fan_latency), ...
