...
n_inputs = eval(get_var('n_inputs','defaults', defaults, varargin{:}));
coeff = eval(get_var('coeff','defaults', defaults, varargin{:}));
...
coeff_bit_width = eval(get_var('coeff_bit_width','defaults', defaults, varargin{:}));
coeff_bin_pt = eval(get_var('coeff_bin_pt','defaults', defaults, varargin{:}));
...
if n_inputs == 0,
  clog('fir_col_init no inputs', 'trace');
  clean_blocks(blk);
...
if bus_input,
    ...
    set_param([blk, '/inbus'], ...
        ...
        'outputWidth', input_width, ...
        'outputBinaryPt', input_bp, ...
...
if async_ops,
    reuse_block(blk, 'dv', 'built-in/inport', 'Position', [0 0 30 16], 'Port', '1');
...
for ctr=1:n_inputs,
    if n_inputs > 1
        ...
        add_line(blk, ['fir_tap',num2str(ctr),'/3'], ['real_sum/1']);
        add_line(blk, ['fir_tap',num2str(ctr),'/4'], ['imag_sum/1']);
    end
end

reuse_block(blk, 'real_sum', 'built-in/outport', 'Position', [900 100+20*n_inputs 930 116+20*n_inputs]);
reuse_block(blk, 'imag_sum', 'built-in/outport', 'Position', [900 200+20*n_inputs 930 216+20*n_inputs]);
