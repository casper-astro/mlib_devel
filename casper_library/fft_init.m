...
defaults = {'FFTSize', 4,  ...
    'input_bit_width', 18, 'coeff_bit_width', 18, 'n_inputs', 1, ... 
...
%generate multiplier specification 
...
vec_biplex_str = tostring(vec_biplex);
vec_direct_str = tostring(vec_direct);
...
if n_inputs < 1,
    ...
    reuse_block(blk, name, 'casper_library/FFTs/fft_biplex', ...
	    ...
	    'conv_latency', num2str(conv_latency), 'dsp48_adders', tostring(dsp48_adders), ...
	    ...
	    'specify_mult', tostring(specify_mult), 'mult_spec', vec_biplex_str, ...
        'Position', pos);
...
elseif n_inputs == FFTSize,
    ...
    reuse_block(blk, 'fft_direct', 'casper_library/FFTs/fft_direct', ...
        ...
        'arch', tostring(arch), 'opt_target', tostring(opt_target), ...
        'coeffs_bit_limit', num2str(coeffs_bit_limit), ...
	    'specify_mult', tostring(specify_mult), 'mult_spec', vec_direct_str, ...
