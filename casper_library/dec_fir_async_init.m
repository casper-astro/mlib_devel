...
n_inputs = eval(get_param(blk, 'n_inputs'));
coeff = eval(get_param(blk, 'coeff'));
...
reuse_block(blk, 'sync_in', 'built-in/inport', ...
    'Position', [0 0 30 16], 'Port', '1');
reuse_block(blk, 'sync_goto', 'built-in/goto', ...
        'GotoTag', 'sync_in', 'showname', showname, ...
        'Position', [60, 0, 60+sizex_goto, 0+sizey_goto]);
add_line(blk, 'sync_in/1', 'sync_goto/1');

% dv in
reuse_block(blk, 'dv_in', 'built-in/inport', ...
    'Position', [0 50 30 66], 'Port', '2');
