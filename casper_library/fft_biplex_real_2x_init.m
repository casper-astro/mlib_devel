...
twiddle_sharing     = get_var('twiddle_sharing', 'defaults', defaults, varargin{:});
twiddle_decimation  = get_var('twiddle_decimation', 'defaults', defaults, varargin{:});
...
reuse_block(blk, 'even_pol_debus', 'casper_library_flow_control/bus_expand', ...
  ...
  'outputWidth', num2str(n_bits_out), 'outputBinaryPt', num2str(bin_pt), ...
  'outputArithmeticType', '1', 'Position', [510 74 605 116+(n_inputs-1)*60]);

reuse_block(blk, 'odd_pol_debus', 'casper_library_flow_control/bus_expand', ...
  'mode', 'divisions of equal size', 'outputNum', num2str(n_inputs), ...
  'outputWidth', num2str(n_bits_out), 'outputBinaryPt', num2str(bin_pt), ...
  'outputArithmeticType', '1', 'Position', [510 74+(n_inputs*60) 605 116+(((n_inputs*2)-1)*60)]);
...
%TODO
%    'twiddle_sharing', twiddle_sharing, ...
%    'twiddle_decimation', twiddle_decimation);
