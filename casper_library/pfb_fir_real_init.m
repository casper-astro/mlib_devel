...
%            propagate_vars([blk,'/',name],'defaults', defaults, varargin{:});
        ...
        % is used to determine how much bit growth is really needed.  The gain of
        ...
        % pfb_fir.  The products, partial sums, and final sum throughout the
        % pfb_fir (including the adder tree) need not accomodate any more bit
        % growth than the absolute maximum gain requires, provided that any
        % "overflow" is ignored (i.e. set to "Wrap").  This works thanks to the
        % wonders of modulo math.  Note that the "gain" for typical signals will
        % be different (less) than the absolute maximum gain of each filter.  For
        % Gaussian noise, the gain of a filter is the square root of the sum of the
        % squares of the coefficients (aka root-sum-squares or RSS).
        ...
        % Compute adder output width and binary point.  We know that the adders
        % in the adder tree need to have (bit_growth+1) non-fractional bits to
        % accommodate the maximum gain.  The products from the taps will have
        % (BitWidthIn+CoeffBitWidth-2) fractional bits.  We will preserve them
        % through the adder tree.
