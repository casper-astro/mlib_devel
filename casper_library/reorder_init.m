...
defaults = {'map', [0 7 1 3 2 5 6 4], 'map_latency', 2, 'bram_latency', 1, ...
  'n_inputs', 1, 'double_buffer', 0, 'bram_map', 'on'};
...
reuse_block(blk, 'delay_valid', 'xbsIndex_r4/Delay', 'reg_retiming', 'on', ...
    'Position', [860  80  900  100], 'latency', num2str(bram_latency+(double_buffer*2)));
...
reuse_block(blk, 'post_sync_delay', 'xbsIndex_r4/Delay', ...
    'reg_retiming', 'on', 'Position', [860  5  900  25], 'latency', num2str(bram_latency+(double_buffer*2)));
...
if order ~= 1,
  if order == 2,
    reuse_block(blk, 'Counter', 'xbsIndex_r4/Counter', ...
        'Position', [95   base   145   base+55], 'n_bits', num2str(map_bits + order_bits), 'cnt_type', 'Count Limited', ...
...
if order == 1,
    for cnt=1:n_inputs,
        ...
        reuse_block(blk, ['delay_din_bram', num2str(cnt-1)], 'casper_library_delays/delay_bram_en_plus', ...
            'DelayLen', 'length(map)', 'bram_latency', 'bram_latency', ...
...
elseif double_buffer == 0,
  ...
  % Add dynamic wires
  for cnt=1:n_inputs
      add_line(blk, ['we_expand/',num2str(cnt)], ['bram',num2str(cnt-1),'/3']);
      add_line(blk, ['addr_expand/',num2str(cnt)], ['bram',num2str(cnt-1),'/1']);
  ...
  % for order greater than 2, we use a more optimal bram configuration
    ...
    reuse_block(blk, 'Counter', 'xbsIndex_r4/Counter', ...
        'Position', [80  base+300   120   base+340], 'n_bits', num2str(map_bits), 'cnt_type', 'Free Running', ...
        'arith_type', 'Unsigned', 'en', 'on', 'rst', 'on');
    add_line(blk, 'sync/1', 'Counter/1');
    add_line(blk, 'en/1', 'Counter/2');
    
    % logic to cater for resyncing
    reuse_block(blk, 'dsync', 'xbsIndex_r4/Delay', 'reg_retiming', 'on', 'latency', '1', 'Position', [85 base+50 115 base+70]);
    ...
    reuse_block(blk, 'current_map', 'xbsIndex_r4/Dual Port RAM', ...
      'latency', num2str(map_latency), ...
      'depth', num2str(map_length), 'initVector', ['[0:',num2str((2^map_bits)-1),']'], ...
      'optimize', optimize, 'distributed_mem', map_memory_type, ...
    ...
    if strcmp(bram_map, 'on'),
      reuse_block(blk, 'always', 'xbsIndex_r4/Constant', ...
        ...
        'Position', [265 base+197 280 base+213]);
      add_line(blk, 'always/1', 'current_map/3'); 
      ...
      reuse_block(blk, 'always', 'xbsIndex_r4/Constant', ...
        'const', '1', 'arith_type', 'Boolean', 'explicit_period', 'on', 'period', '1', ...
        'Position', [265 base+222 280 base+238]);
      add_line(blk, 'always/1', 'current_map/3'); 
    end
  
    reuse_block(blk, 'daddr1', 'xbsIndex_r4/Delay', 'reg_retiming', 'on', 'latency', num2str(map_latency), ...
      'Position', [330 base+310 370 base+330]);
    ...
    reuse_block(blk, 'map_mod', 'xbsIndex_r4/ROM', ...
      'depth', num2str(map_length), 'initVector', 'map', 'latency', num2str(map_latency), ...
      'arith_type', 'Unsigned', 'n_bits', num2str(map_bits), 'bin_pt', '0', 'optimize', optimize, ...
      'distributed_mem', map_memory_type, 'Position', [510  base+194   560    base+246]);
    add_line(blk, 'map_mux/1', 'map_mod/1');
    add_line(blk, 'map_mod/1', 'current_map/2', 'autorouting', 'on');

    reuse_block(blk, 'daddr2', 'xbsIndex_r4/Delay', 'reg_retiming', 'on', 'latency', num2str(1+map_latency), ...
      'Position', [440 base+310 560 base+330]);
    add_line(blk, 'daddr1/1', 'daddr2/1');
    add_line(blk, 'daddr2/1', 'current_map/1', 'autorouting', 'on');
