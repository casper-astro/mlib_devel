function casper_library_downconverter_init()
	...
	set_param([blk,'/cosin'], ...
		...
		'conv_latency', sprintf('3'), ...
		'fraction', sprintf('full'), ...
		'pack', sprintf('on'), ...
		'bram', sprintf('BRAM'), ...
...
function cosin_mask(blk)
	...
	set_param(blk, ...
		...
		'MaskPromptString', sprintf('first output function|second output function|lookup table size (2^?)|bit width|binary point|bram latency|large adder latency|mux latency|negate latency|convert latency|fraction of cosine period in lookup|pack lookup values in same output word|BRAM implementation'), ...
		'MaskStyleString', sprintf('popup(sin|cos|-sin|-cos),popup(sin|cos|-sin|-cos),edit,edit,edit,edit,edit,edit,edit,edit,popup(quarter|half|full),checkbox,popup(distributed RAM|BRAM)'), ...
		'MaskTabNameString', sprintf('basic,basic,basic,basic,basic,latency,latency,latency,latency,latency,implementation,implementation,implementation'), ...
		'MaskCallbackString', sprintf('||||||||||||'), ...
		...
		'MaskVariables', sprintf('output0=&1;output1=&2;table_bits=@3;n_bits=@4;bin_pt=@5;bram_latency=@6;add_latency=@7;mux_latency=@8;neg_latency=@9;conv_latency=@10;fraction=&11;pack=&12;bram=&13;'), ...
		'MaskValueString', sprintf('cos|-sin|0|18|17|1|1|1|1|3|full|on|BRAM'), ...
		'BackgroundColor', sprintf('[0.501961, 1.000000, 0.501961]'), ...
		'MaskDisplay', sprintf('if ~((table_bits == 0) || table_bits < 3 && ~(strcmp(fraction, ''full'') && strcmp(pack,''on''))),\n    color(''black'');port_label(''input'',1,''theta'');\n    color(''black'');port_label(''input'',2,''misci'');\n    color(''black'');port_label(''output'',1,output0);\n    color(''black'');port_label(''output'',2,output1);\n    color(''black'');port_label(''output'',3,''misco'');\n    color(''black'');disp([''z^{-'',num2str(add_latency+mux_latency+conv_latency+bram_latency+neg_latency+mux_latency),''}''],''texmode'',''on'');\nend'));
