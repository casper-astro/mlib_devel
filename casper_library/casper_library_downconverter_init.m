function casper_library_downconverter_init()
	...
	add_block('built-in/SubSystem', [blk,'/fir_dbl_tap_async']);
	fir_dbl_tap_async_gen([blk,'/fir_dbl_tap_async']);
	set_param([blk,'/fir_dbl_tap_async'], ...
		'factor', sprintf('0.073384000000000004781952611665474'), ...
		'add_latency', sprintf('1'), ...
		'mult_latency', sprintf('2'), ...
		'coeff_bit_width', sprintf('0'), ...
		'coeff_bin_pt', sprintf('17'), ...
		'async', sprintf('off'), ...
		'Position', sprintf('[222 405 272 472]'), ...
		'Tag', sprintf(''));

	add_block('built-in/SubSystem', [blk,'/fir_dbl_col_async']);
	fir_dbl_col_async_gen([blk,'/fir_dbl_col_async']);
	set_param([blk,'/fir_dbl_col_async'], ...
		'n_inputs', sprintf('0'), ...
		'coeff', sprintf('0.10000000000000000555111512312578'), ...
		'add_latency', sprintf('2'), ...
		'mult_latency', sprintf('3'), ...
		...
		'input_bp', sprintf('0'), ...
		'input_type', sprintf('Signed'), ...
		'Position', sprintf('[319 405 354 520]'), ...
		'Tag', sprintf(''));
	...
	set_param([blk,'/fir_col_async'], ...
		...
		'input_bp', sprintf('0'), ...
		'input_type', sprintf('Signed'), ...
		'Position', sprintf('[401 405 436 520]'), ...
		'Tag', sprintf(''));
...
function dec_fir_async_mask(blk)
	...
	set_param(blk, ...
		...
		'MaskDescription', sprintf('FIR filter which can handle multiple time samples in parallel and decimates down to 1 time sample.  If coefficients are symmetric, will automatically fold before multiplying.'), ...
		'MaskPromptString', sprintf('Number of Parallel Streams|Coefficients|Bit Width Out|Quantization Behavior|Add Latency|Mult Latency|Convert latency|Coefficient bit width|Coefficient binary point|Absorb adders into DSP slices|Adder implementation|Asynchronous ops|Bus input|Input bitwidth|Input binary point|Input datatype'), ...
		'MaskStyleString', sprintf('edit,edit,edit,popup(Truncate|Round  (unbiased: +/- Inf)|Round  (unbiased: Even Values)),edit,edit,edit,edit,edit,checkbox,popup(Behavioral|Fabric|DSP48),checkbox,checkbox,edit,edit,popup(Signed|Unsigned)'), ...
		...
		'MaskEnableString', sprintf('on,on,on,on,on,on,on,on,on,on,on,on,on,off,off,off'), ...
		'MaskVisibilityString', sprintf('on,on,on,on,on,on,on,on,on,on,on,on,on,on,on,on'), ...
		'MaskToolTipString', sprintf('on,on,on,on,on,on,on,on,on,on,on,on,on,on,on,on'), ...
		'MaskVariables', sprintf('n_inputs=@1;coeff=@2;n_bits=@3;quantization=&4;add_latency=@5;mult_latency=@6;conv_latency=@7;coeff_bit_width=@8;coeff_bin_pt=@9;absorb_adders=&10;adder_imp=&11;async=&12;bus_input=&13;input_width=@14;input_bp=@15;input_type=&16;'), ...
		'MaskValueString', sprintf('0|0.10000000000000000555111512312578|8|Round  (unbiased: +/- Inf)|1|2|2|25|24|on|DSP48|off|off|0|0|Signed'), ...
		'BackgroundColor', sprintf('[0.501961, 1.000000, 0.501961]'), ...
		'Tag', sprintf('casper:dec_fir'));
...
function fir_tap_async_mask(blk)
	...
	set_param(blk, ...
		...
		'MaskDescription', sprintf('Multiplies input data with factor specified.'), ...
		'MaskHelp', sprintf('eval(''xlWeb(''''http://casper.berkeley.edu/wiki/Fir_tap'''')'')'), ...
		'MaskPromptString', sprintf('Factor|Mult latency|Coefficient bit width|Coefficient binary point|Asynchronous ops'), ...
		'MaskStyleString', sprintf('edit,edit,edit,edit,checkbox'), ...
		'MaskCallbackString', sprintf('||||'), ...
		'MaskEnableString', sprintf('on,on,on,on,on'), ...
		'MaskVisibilityString', sprintf('on,on,on,on,on'), ...
		'MaskToolTipString', sprintf('on,on,on,on,on'), ...
		'MaskVariables', sprintf('factor=@1;mult_latency=@2;coeff_bit_width=@3;coeff_bin_pt=@4;async=&5;'), ...
		'MaskValueString', sprintf('1|2|0|24|off'), ...
		'BackgroundColor', sprintf('[0.501961, 0.501961, 0.501961]'));
...
function fir_dbl_tap_async_gen(blk)

	fir_dbl_tap_async_mask(blk);
	fir_dbl_tap_async_init(blk);
	set_param(blk, ...
		'MaskInitialization', sprintf('fir_dbl_tap_async_init(gcb);'));

end % fir_dbl_tap_async_gen

function fir_dbl_tap_async_mask(blk)

	set_param(blk, ...
		'Mask', sprintf('on'), ...
		'MaskSelfModifiable', sprintf('on'), ...
		'MaskType', sprintf('fir_dbl_tap_async'), ...
		'MaskHelp', sprintf('eval(''xlWeb(''''http://casper.berkeley.edu/wiki/Fir_dbl_tap'''')'')'), ...
		'MaskPromptString', sprintf('factor|Add latency|Mult latency|Coefficient bit width|Coefficient binary point |Asynchronous ops'), ...
		'MaskStyleString', sprintf('edit,edit,edit,edit,edit,checkbox'), ...
		'MaskCallbackString', sprintf('|||||'), ...
		'MaskEnableString', sprintf('on,on,on,on,on,on'), ...
		'MaskVisibilityString', sprintf('on,on,on,on,on,on'), ...
		'MaskToolTipString', sprintf('on,on,on,on,on,on'), ...
		'MaskVariables', sprintf('factor=@1;add_latency=@2;mult_latency=@3;coeff_bit_width=@4;coeff_bin_pt=@5;async=&6;'), ...
		'MaskValueString', sprintf('0.073384000000000004781952611665474|1|2|0|17|off'), ...
		'BackgroundColor', sprintf('[0.501961, 0.501961, 0.501961]'));

end % fir_dbl_tap_async_mask

function fir_dbl_tap_async_init(blk)

end % fir_dbl_tap_async_init

function fir_dbl_col_async_gen(blk)

	fir_dbl_col_async_mask(blk);
	fir_dbl_col_async_init(blk);
	set_param(blk, ...
		'MaskInitialization', sprintf('fir_dbl_col_async_init(gcb);'));

end % fir_dbl_col_async_gen

function fir_dbl_col_async_mask(blk)

	set_param(blk, ...
		'Mask', sprintf('on'), ...
		'MaskSelfModifiable', sprintf('on'), ...
		'MaskType', sprintf('fir_dbl_col_async'), ...
		'MaskHelp', sprintf('eval(''xlWeb(''''http://casper.berkeley.edu/wiki/Fir_dbl_col'''')'')'), ...
		'MaskPromptString', sprintf('Inputs|Coefficients|Add latency|Mult Latency|Coefficient bit width|Coefficient binary point|Implement first stage of adder trees on output as behavioural HDL|Adder implementation|Asynchronous ops|Bus input|Input bitwidth|Input binary point|Input datatype'), ...
		'MaskStyleString', sprintf('edit,edit,edit,edit,edit,edit,checkbox,popup(Behavioral|Fabric|DSP48),checkbox,checkbox,edit,edit,popup(Signed|Unsigned)'), ...
		'MaskCallbackString', sprintf('|||||||||onoff = get_param(gcb, ''bus_input'');\nset_param_state(gcb, ''input_width'', onoff)\nset_param_state(gcb, ''input_bp'', onoff)\nset_param_state(gcb, ''input_type'', onoff)|||'), ...
		'MaskEnableString', sprintf('on,on,on,on,on,on,on,on,on,on,off,off,off'), ...
		...
		'MaskVariables', sprintf('n_inputs=@1;coeff=@2;add_latency=@3;mult_latency=@4;coeff_bit_width=@5;coeff_bin_pt=@6;first_stage_hdl=&7;adder_imp=&8;async=&9;bus_input=&10;input_width=@11;input_bp=@12;input_type=&13;'), ...
		'MaskValueString', sprintf('0|0.10000000000000000555111512312578|2|3|25|24|off|Fabric|off|off|0|0|Signed'), ...
		'BackgroundColor', sprintf('[0.501961, 0.501961, 0.501961]'));

end % fir_dbl_col_async_mask

function fir_dbl_col_async_init(blk)

end % fir_dbl_col_async_init
