...
% % Drawing the fixed blocks in the design.Includes the sync delays at the top
    ...
    reuse_block(blk,'sync_delay3','casper_library/Delays/sync_delay',...
                'DelayLen', '12', ...
                'Position',[695 50 740 80]);
            
    reuse_block(blk,'sync_delay4','casper_library/Delays/sync_delay',...
                'DelayLen', '1', ...
                'Position',[795 50 840 80]);
            
    reuse_block(blk,'sync_delay5','casper_library/Delays/sync_delay',...
                'DelayLen', '2', ...
...
% %  %%%%%%%%%% number of inputs.
 ...
 y2 = 331;
  for i=1:n_input
     ...
     reuse_block(blk,name,'casper_library/Delays/Fine_del_sub_blk',...
     ...
     reuse_block(blk,name,'casper_library/Delays/Fringe_sub_blk',...
    ...
    reuse_block(blk,name,'casper_library/Delays/Datatype_Conv_Cos',...
                'theta_binary_bits', num2str(theta_binary_bits),...
                'fft_bits',num2str(fft_bits),...
                'Position',[795 y1+106+((i-1)*455) 835 y2+39+((i-1)*455)]);
             
   name = ['Datatype_Conv_Sine', num2str(i)];   
   reuse_block(blk,name,'casper_library/Delays/Datatype_Conv_Sine',...
                'theta_binary_bits', num2str(theta_binary_bits),...
                'fft_bits',num2str(fft_bits),...
