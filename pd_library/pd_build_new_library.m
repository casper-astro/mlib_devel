...
% This script must be ran from pd_library folder
% input: source_dir = char array, source directory where models are stored
% input: lib_name = char array, name of new library
% With no arguments:
% source_dir defaults to $MLIB_DEVEL_PATH/pd_library/pd_models
% lib_name defaults to pd_library
% i.e., by default update this repository's library with this repository's
% blocks.
function pd_build_new_library(source_dir,lib_name)
    warning('off','all');
    if nargin == 0
        source_dir = fullfile(getenv('MLIB_DEVEL_PATH'), 'pd_library', 'pd_models');
        lib_name = 'pd_library';
    end;
    disp(source_dir)
    ...
    if isloaded == 4
        ...
        if strcmpi(button,'Yes')
            ...
            %disp('Setting EnableLBRepository');
            %disp(lib_name);
        else
            return;
        end
