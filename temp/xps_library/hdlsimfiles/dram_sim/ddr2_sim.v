...
module `mod_name (
    ...
    parameter IP_CLOCK = 200.0;
    localparam TIMESCALER = 1000.0/IP_CLOCK;
        ...
        begin
            ...
            if (found)
                data = MEM[mem_index];
                   ...
                   // DROZ: modified to get rid of warnings when we read memory loaction that have never been written
                //data = {8*DQ_BITS{1'bx}};
                data = {8*DQ_BITS{1'b0}};
        end
    endtask
        ...
        begin
            ...
            if (!trfc_max_violation) begin
                if (trfc_chk > 0) if ($time - trfc_chk > trfc_max) begin
                    $display ("%m: at time %t ERROR: tRFC maximum violation, chk %t max %t", $time, trfc_chk, trfc_max);
                    if (!no_halt) $stop(0);
            ...
            if (cke_in) begin
                case (cmd)
                    ...
                    `PRE : begin // Precharge
                        if (!power_up_done && (tcke_chk + 400000/10.0 > $time))
                            $display ("%m at time %t WARNING: A 400 ns delay is required before Precharge All can be issued after CKE transitions HIGH. %t", $time, tcke_chk);
    ...
    always @(cs_n_in) begin
        if (cke_in_one_clk_back) begin
            ...
            if (cs_n_event + tipw*tck > $time)
                $display ("%m: at time %t ERROR: tIPW violation on CS_N by %t, nominal %t * %t", $time, cs_n_event + tipw*tck - $time, tipw, tck);
        end
        cs_n_event <= $time;
    end
