...
function fft_2d_init(blk, varargin)
% Initialize and configure the complex 2D FFT.
%
% This script is based on (read: a hacked version of) fft_init.m
% by Aaron Parsons and Terry Filiba, CASPER, U.C. Berkeley 
%
% varargin = {'varname', 'value', ...} pairs
% 
% Valid varnames for this block are:
% Nx = Size of the x dimension FFT (2^Nx points).
% Ny = Size of the y dimension FFT (2^Ny points).
...
check_mask_type(blk, 'fft_2d');
munge_block(blk, varargin{:});

Nx = get_var('Nx', 'defaults', defaults, varargin{:});
Ny = get_var('Ny', 'defaults', defaults, varargin{:});
...
if( strcmp(specify_mult, 'on') && length(mult_spec) ~= (Nx + Ny) ),
    error('fft_init.m: Multiplier use specification for stages does not match FFT x+y size');
    return
end
